...
class PackageModel:
    ...
    def init(self, package_path):
        ...
        self.package_name_warning = False  # 当包名不是P加5位数字时，为True
    ...
    def _classify(self):
        ...
        """
        # 按照规范检测包名
        if self.package_name != re.search(r'[P]\d{4}', self.package_name).group():
            self.package_name_warning = True
    ...
    def _judge_if_normal_package(self):
        ...
        # 包名规范检测
        if self.package_name_warning:
            self.is_normal_package = False
            self.abnormal_reason.append("包名不规范:" + json.dumps(self.package_name))
